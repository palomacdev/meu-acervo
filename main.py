...
# Lista de origens permitidas (endereços que podem acessar a API)
origins = [
    "http://localhost:3000", 
    "https://super-duper-space-acorn-gv5jw6vq9p6cp45x-3000.app.github.dev/" # Endereço do React localmente
...
def update_media(media_id: int, media_update: MediaCreate, db: Session = Depends(get_db)):
    ...
    # Atualiza os campos do objeto do banco de dados com os dados recebidos
...
def delete_media(media_id: int, db: Session = Depends(get_db)):
    """
    Deleta um item de mídia pelo seu ID.
    """
    db_media = db.query(Media).filter(Media.id == media_id).first()
    if db_media is None:
        raise HTTPException(status_code=404, detail="Mídia não encontrada")
